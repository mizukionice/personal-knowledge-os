...
```
日付: 2026-08-08
対象: 初の実書籍フルパイプライン検証（「論語と算盤」渋沢栄一、iPhone写真134ページ）
仮説: 実書籍規模（100ページ超）でもアップロード→VLM解析→Knowledge化が完走する
方法: 本番環境（pkos-web.pages.dev）からJPEG134枚をアップロードし全工程を実行
結果:
- ページ解析: 134/134成功（Opus逐次、約55秒/ページ、計約2時間）。Markdown品質は目視で良好
- 概念抽出が「LLM output was truncated (max_tokens reached)」で失敗 → 数百概念を
  1回のLLM呼び出しで正規化する実装がスケールしない問題。50概念/バッチに分割して解決
- 修正後のKnowledge化再実行: 約45分で完走。chunks 207 / 概念537（mentions 695）/
  links累計374。importance上位は蟹穴主義・士魂商才・義利合一など中核概念で妥当
- コスト実測: ページ解析$12.43（約9.3¢/ページ ≒ M5-03計測と同水準）+ Knowledge化再実行分
発見（インフラ。本番のみで発生しローカルでは再現しない設定漏れ2件）:
- R2バケットCORSに本番オリジン未登録 → 全PUTがプリフライト403で失敗
- 本番Worker secretsが未設定（wrangler secret list が空）→ presignが一律500。
  認証(JWKS)/DB(anon key+RLS)はsecretsなしで動くため他機能では発覚しない
- APIレートリミット60req/分に134ファイルのpresignが衝突 → 429にRetry-Afterを付与し
  クライアントが待って自動再開する方式で解決（上限は緩めない）
判断: パイプラインは実書籍規模で成立。コスト削減候補（PageAnalyzerのSonnet化 or
      Batch API 50%オフ）は品質比較の上で別途判断
```

```
日付: 2026-07-20
対象: M5-03 コスト計測（1冊あたりのVLM/LLM/embedding費用）
方法: UsageMeterで実バッチのトークン使用量を集計。「情報処理基礎」（写真10ページ、
      専門書1章相当）のページをpendingに戻し全工程を再処理、ActionsログのcostラインをGitHub APIで取得
結果（claude-opus-4-8、$5/$25 per MTok）:
- VLM（PageAnalyzer）: 10呼び出し、in=50,167 / out=34,946 tok → $1.1245（約$0.112/ページ）
- LLM（Chunker→ConceptExtractor→RelationExtractor）: 3呼び出し、in=35,156 / out=21,590 tok → $0.7155
- Anthropic合計: **$1.84 / 1冊（10ページ）**（≒$0.184/ページ）
- Embedding（Workers AI @cf/baai/bge-m3）: 別計上。BGE-M3は無償枠/極安のため実質無視できる
- 処理時間: 10ページで約12.5分（VLMのadaptive thinking込み）
- キャッシュは未使用（cacheRead/Write=0）。ページ毎に独立プロンプトのため
判断: 1冊$2弱。改善候補: PageAnalyzerのsystemプロンプトをprompt cachingで固定すれば
      入力トークンを削減可能（M5以降の最適化候補として10_RESEARCHに残す）
```

```
日付: 2026-07-20
対象: M5-02 セキュリティ確認（RLS / R2非公開 / sanitize / rate limit）
方法: 静的レビュー + 実Supabase/R2/Workersへの挙動テスト（scratchpad/sec-check.mjs相当）
結果（挙動テスト 6/6 パス）:
- 認証: 未認証の /v1/documents は401、改竄JWTも401（middleware/auth.tsのJWKS検証）
- RLS: 全7テーブルでRLS有効・ポリシー付き。別ユーザーのJWTでowner書籍GETは404、
  検索でも他ユーザーのchunkに到達しない（results=0）。APIはanonキー+ユーザーJWTで
  接続しRLSを適用（db.ts）。search_chunks/expand_related_chunks RPCもSECURITY INVOKERで
  RLS準拠＋uid明示フィルタの二重防御
- R2非公開: S3エンドポイントへの無認証GETは400（SigV4署名必須）＝匿名読み取り不可。
  公開r2.devドメインは未設定。閲覧はWorkers API経由（認証+RLSでr2_prefix検証）のみ
- sanitize: Document ViewerのMarkdownは rehype-sanitize 適用済み（XSS対策）
- rate limit: 認証済みでも既定60/分を超えると429（middleware/rate-limit.ts）
判断: M5-02のセキュリティ要件を満たす。改善余地: rate limitはisolateローカルMapのため
      複数isolate跨ぎでは厳密でない（TDD既知。必要ならDurable Objects/KVへ）
```
