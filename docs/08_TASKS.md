...
## M1: Auth + Upload
- [ ] M1-01 web: Vite + React + Tailwind + shadcn/ui セットアップ、ルーティング
- [ ] M1-02 web: Login/Signup画面（Supabase Auth）、セッション管理、認証ガード
- [ ] M1-03 api: Hono + JWT検証ミドルウェア + エラーハンドラ + rate limit
- [ ] M1-04 api: documents CRUD（POST/GET/DELETE）
- [ ] M1-05 api: upload-url発行（R2署名付きURL）+ uploads/complete
- [ ] M1-06 web: Library画面（一覧・削除）
- [ ] M1-07 web: Upload画面（複数写真・PDF、並び替え、進捗、リトライ）
- 完了条件: ログインし、写真をアップロードし、Libraryに書籍が並ぶ（処理はまだ）

## M2: Processing Pipeline（最初の価値: 写真→Markdown）
- [ ] M2-01 kps: interfaces.ts（KPS §2の全インターフェース）+ PageAnalysis zodスキーマ
- [ ] M2-02 kps: DocumentParser（PDF→ページ画像。pdfjs or mupdf）
